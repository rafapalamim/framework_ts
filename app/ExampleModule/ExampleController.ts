...
export default class ExampleController implements Controller {
    ...
    register(server: HttpServer): void {
        server.addRoute('get', '/test', async (request: HttpRequest<{}, {}>, response: HttpResponse, next: NextFunction) => {
            const output = await Promise.resolve(1)
            response.status(200).send({ hello: 'true', promise: output, request: request.body })
            next()
        })

        server.addRoute('post', '/test', async (request: HttpRequest<{name: string}, {}>, response: HttpResponse, next: NextFunction) => {
            const output = await Promise.resolve(1)
            console.log(request)
            response.status(200).send({ hello: 'true', promise: output, request: request.body })
            next()
